...
;(function( $, window, document, undefined ){
  ...
  var FlowGallery = function(list, config) {
      ...
      for(i=0; i<itemsLength; i++) {
        ...
        if( $listItem.hasClass('active') ) {
          ...
          if( jQuery(window).width() <= 960 ){
            config.left   = (centerX - options.imagePadding - currentItem.tw * 0.5) + 'px'; 
            config.top    = '0';
            config.width  = currentItem.tw;
            config.height = currentItem.th;
          }
          ...
          if(listHeight !== currentItem.h) {
            if( jQuery(window).width() <= 960 ){
              listHeight = flowItems[0].th;
            } else {
              listHeight = flowItems[0].h;
            }
    ...
    function calculateLeftPosition(current, isBefore) {
      ...
      if (isBefore) {
        if( jQuery(window).width() <= 960 ){
          left -= flowItems[activeIndex].tw*0.5;
        } else {
